...
const contentScriptLinks = ['scripts/jquery-3.7.1.min.js'];
...
function addToCartTmall(itemMain, itemMain2) {
  ...
  if (purchasePanel) {
    ...
    const addToCartLoading = document.getElementById('addToCartLoading');

    // Fade out/in using CSS (you can adapt this to your class-based fade animation if needed)
    addToCart.style.transition = 'opacity 0.2s';
    addToCart.style.opacity = 0;
    setTimeout(() => {
      addToCart.style.display = 'none';
      addToCartLoading.style.display = 'block';
      addToCartLoading.style.opacity = 0;
      setTimeout(() => addToCartLoading.style.opacity = 1, 10);

      const url = window.location.href;
      const link = url;
      let keterangan_1 = '';
      let keterangan_2 = '';
      const sellerEl = document.querySelector('span[class*="--shopName--"]');
      const productNameEl = document.querySelector('h1[class*="--mainTitle--"]');
      const qtyEl = document.querySelector('input[class*="--countValue--"]');
      const imgEl = document.querySelector('div[class*="--mainPicWrap--"] img');
      const priceEl = document.querySelector('span[class*="--text--"]');


      let seller = sellerEl?.getAttribute('title') || '';
      let productName = productNameEl?.getAttribute('title') || '';
      let qty = qtyEl ? parseInt(qtyEl.value) : 1;
      let img_link = imgEl?.getAttribute('src') || '-';
      let price = priceEl ? parseFloat(priceEl.textContent.replace(/[^\d.]/g, '')).toFixed(2) : 0;

      const items = document.querySelectorAll('div[class*="--skuItem--"]');

      items.forEach((element, index) => {
        const selected = element.querySelector('div[class*="--valueItem--"][class*="--isSelected--"] span[title]');
      ...
      const save = new ItemDucking().save(data);
      if (save) {
        addToCartLoading.style.transition = 'opacity 0.2s';
        addToCartLoading.style.opacity = 0;
        setTimeout(() => {
          addToCartLoading.style.display = 'none';
          addToCart.style.display = 'block';
          addToCart.style.opacity = 0;
          setTimeout(() => addToCart.style.opacity = 1, 10);
        }, 200);
      }
